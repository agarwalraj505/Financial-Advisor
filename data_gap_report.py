...
GAP_RULES = {
    "price": lambda a, candidate: not any(_number(a.get(field)) > 0 for field in
                                            ("live_current_price", "current_price_eur", "manual_current_price", "latest_price")),
    "symbol": lambda a, candidate: str(a.get("asset_type", "")).lower() != "cash" and not str(a.get("resolved_price_symbol") or a.get("price_symbol") or "").strip(),
...
def _attempt_summary(asset: dict) -> tuple[str, str, str]:
    ...
    failure = str(failures[-1].get("details", failures[-1].get("detail", ""))) if failures else ""
